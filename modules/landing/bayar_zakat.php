...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $jumlahZakat = $_POST['jumlah_zakat'];
    $metodePembayaran = $_POST['metode_pembayaran'];

    $stmt = $conn->prepare("INSERT INTO pembayaran_zakat (user_id, jumlah_zakat, metode_pembayaran, tanggal) VALUES (?, ?, ?, NOW())");
    $stmt->bind_param("ids", $idUser, $jumlahZakat, $metodePembayaran);
    $stmt->execute();
...
<body class="bg-white text-gray-800 font-sans">
    ...
    <section class="max-w-lg mx-auto my-16">
        <h2 class="text-3xl text-center text-green-700 mb-4">Pembayaran Zakat</h2>
        <form method="POST">
            <div class="mb-4">
                <label for="jumlah_zakat" class="block text-sm font-medium text-gray-700">Jumlah Zakat</label>
                <input type="text" name="jumlah_zakat" id="jumlah_zakat" class="mt-2 p-2 border border-gray-300 rounded w-full" required />
            </div>
            ...
            <div class="flex justify-center">
                <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700">Bayar Zakat</button>
            </div>
        </form>
    </section>
    ...
    <script>
        ...
        const inputJumlah = document.getElementById("jumlah_zakat");
