<?php
session_start();
require_once './database/koneksi.php'; // Koneksi ke database
...
// AMBIL DATA METODE PEMBAYARAN — INI PENTING
$query = "SELECT * FROM metode_pembayaran";
$result = $conn->query($query);
$metodePembayaranData = $result->fetch_all(MYSQLI_ASSOC);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $jumlahInfaq = $_POST['jumlah_infaq'];
    $metodePembayaran = $_POST['metode_pembayaran'];

    $stmt = $conn->prepare("INSERT INTO pembayaran_infaq (user_id, jumlah_infaq, metode_pembayaran, tanggal) VALUES (?, ?, ?, NOW())");
    $stmt->bind_param("dss", $idUser, $jumlahInfaq, $metodePembayaran);
    $stmt->execute();
...
<body class="bg-white text-gray-800 font-sans">
    ...
    <header class="bg-green-600 text-white shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold">Pembayaran Infaq</h1>
        </div>
    </header>

    <!-- Form Pembayaran Infaq -->
    <section class="max-w-lg mx-auto my-16">
        <h2 class="text-3xl text-center text-green-700 mb-4">Pembayaran Infaq</h2>
        <form method="POST">
            <div class="mb-4">
                <label for="jumlah_infaq" class="block text-sm font-medium text-gray-700">Jumlah Infaq</label>
                <input type="text" name="jumlah_infaq" id="jumlah_infaq" class="mt-2 p-2 border border-gray-300 rounded w-full" required />
            </div>
            ...
            <div class="flex justify-center">
                <button type="submit" class="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700">Bayar Infaq</button>
            </div>
        </form>
    </section>
    ...
    <script>
        ...
        const inputJumlah = document.getElementById("jumlah_infaq");
